...
class AdminBorrowingAPITests(TestCase):
    ...
    @mock.patch("borrowings.signals.send_telegram_message", return_value=None)
    def test_list_with_user_id_filter(self, mock_send_message):
        book = sample_book()
        ...
        Borrowing.objects.create(
            expected_return_date=timezone.now().date() + timedelta(days=11),
            book=book,
            user=non_admin_user,
        )
...
class AuthenticatedBorrowingAPITests(TestCase):
    ...
    @mock.patch("borrowings.signals.send_telegram_message", return_value=None)
    def test_list_with_is_active_filter(self, mock_send_message):
        book = sample_book()
        Borrowing.objects.create(
            expected_return_date=timezone.now().date() + timedelta(days=10),
            book=book,
    ...
    @mock.patch("borrowings.signals.send_telegram_message", return_value=None)
    def test_borrowing_creation_fails_when_book_inventory_zero(self, mock_send_message):
        book = sample_book()
        book.inventory = 0
        book.save()
    ...
    @mock.patch("borrowings.signals.send_telegram_message", return_value=None)
    def test_expected_return_date_before_borrow_date_forbidden(self, mock_send_message):
        book = sample_book()
        payload = {
            "expected_return_date": (
    ...
    @mock.patch("borrowings.signals.send_telegram_message", return_value=None)
    def test_book_inventory_decrements_on_borrowing_creation(self, mock_send_message):
        book = sample_book()
    ...
    @mock.patch("borrowings.signals.send_telegram_message", return_value=None)
    def test_borrowing_cannot_be_returned_if_already_returned(self, mock_send_message):
        book = sample_book()
        Borrowing.objects.create(
            user=self.user,
            book=book,
    ...
    @mock.patch("borrowings.signals.send_telegram_message", return_value=None)
    def test_borrowing_can_be_returned_and_inventory_increases(self, mock_send_message):
        book = sample_book()
